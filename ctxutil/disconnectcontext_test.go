...
func TestDisconnectContext(t *testing.T) {
	...
	ctx2, cancel2 := DisconnectContextWithTimeout(ctx1, 300*time.Millisecond)
	defer cancel2()
	require.EqualValues("bar", ctx2.Value(key("foo")))
	ctx3, cancel3 := DisconnectContextWithTimeout(ctx1, 700*time.Millisecond)
	defer cancel3()
